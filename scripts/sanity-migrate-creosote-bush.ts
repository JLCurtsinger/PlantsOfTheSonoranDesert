...
/**
 * Migration script for Creosote Bush plant to Sanity.
 * 
 * This script:
 * 1. Loads local Creosote Bush plant data
 * 2. Finds or creates the plant document in Sanity (slug: "creosote-bush")
 ...
 * - detailSections[].description is never blank (fallback: local description -> title/alt -> filename)
...
function deriveDetailSectionDescription(
  ...
  // Handle common patterns like "creosote-bush" -> "Creosote Bush."
...
async function main() {
  console.log(`\n🌵 Migrating Creosote Bush to Sanity`)
  console.log(`   Project: ${projectId}`)
  console.log(`   Dataset: ${dataset}`)
  console.log(`   Slug: ${SLUG}\n`)

  // Load local plant data
  const localPlant = creosoteBush
  ...
  // detailSections maps to detailSections (if exists) or galleryDetails
  if (localPlant.detailSections) {
    localPlant.detailSections.forEach(section => imagePaths.add(section.src))
  } else if (localPlant.galleryDetails) {
    // Use galleryDetails as detailSections if detailSections doesn't exist
  ...
  const sourceSections = localPlant.detailSections || 
    (localPlant.galleryDetails?.map(item => ({
      src: item.src,
      alt: item.alt || '',
      title: item.title,
  ...
  const detailSectionsRefs = sourceSections.map(section => {
  ...
  const beforeState = await client.fetch(
    `*[_type=="plant" && slug.current=="creosote-bush"][0]{
