...
/**
 * Migration script for Palo Verde plant to Sanity.
 * 
 * This script:
 * 1. Loads local Palo Verde plant data
 * 2. Finds or creates the plant document in Sanity (slug: "palo-verde")
...
/**
 * Derives a stable key from an image path for gallery and detailSections.
 */
...
async function main() {
  console.log(`\n🌵 Migrating Palo Verde to Sanity`)
  console.log(`   Project: ${projectId}`)
  console.log(`   Dataset: ${dataset}`)
  console.log(`   Slug: ${SLUG}\n`)

  // Load local plant data
  const localPlant = paloVerde
  ...
  const beforeState = await client.fetch(
    `*[_type=="plant" && slug.current=="palo-verde"][0]{
  ...
  if (verification.missingDetailTitles > 0) {
    console.warn(`\n⚠️  WARNING: Some detail sections are missing titles!`)
    process.exit(1)
  }
  ...
  if (verification.missingGalleryKeys > 0 || verification.missingDetailKeys > 0) {
    ...
    console.warn(`   missingGalleryKeys: ${verification.missingGalleryKeys}`)
    console.warn(`   missingDetailKeys: ${verification.missingDetailKeys}`)
    process.exit(1)
  }
