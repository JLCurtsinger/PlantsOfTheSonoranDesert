...
/**
 * Migration script for Barrel Cactus plant to Sanity.
 * 
 * This script:
 * 1. Loads local Barrel Cactus plant data
 * 2. Finds or creates the plant document in Sanity (slug: "barrel-cactus")
 ...
 * - detailSections[].description is never blank (fallback: local description -> alt/title sentence -> cleaned filename sentence)
 * - Idempotent: re-running updates the same doc by slug
 * - Resilient: reuses existing asset refs if local images are missing
 * 
...
function deriveDetailSectionDescription(
  ...
  // Handle common patterns like "barrel-cactus-branches" -> "Barrel Cactus branches."
...
async function main() {
  console.log(`\n🌵 Migrating Barrel Cactus to Sanity`)
  console.log(`   Project: ${projectId}`)
  console.log(`   Dataset: ${dataset}`)
  console.log(`   Slug: ${SLUG}\n`)

  // Load local plant data
  const localPlant = barrelCactus
  ...
  // galleryDetails maps to detailSections
  if (localPlant.galleryDetails) {
    localPlant.galleryDetails.forEach(section => imagePaths.add(section.src))
  }
  ...
  // Prepare detailSections from galleryDetails
  // HARDENED: Each detailSection must have _key, title (never blank), and description (never blank)
  const detailSectionsRefs = (localPlant.galleryDetails || []).map(section => {
  ...
  const beforeState = await client.fetch(
    `*[_type=="plant" && slug.current=="barrel-cactus"][0]{
  ...
  console.log(`   Assets reused: ${reusedCount}`)
